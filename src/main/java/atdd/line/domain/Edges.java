...
public class Edges {
    ...
    private Station findNextStationOf(List<Edge> edges, Station firstStation) {
        return edges.stream()
                .filter(it -> firstStation.equals(it.getTargetStation()))
                .map(it -> it.getSourceStation())
                .findFirst()
                .orElse(null);
    }
    ...
    public Edges removeStation(Station station) {
        ...
        Edge newEdge = Edge.builder()
                .sourceStation(getSourceStationOf(station))
                .targetStation(getTargetStationOf(station))
                .distance(sum(replaceEdge))
                .build();
