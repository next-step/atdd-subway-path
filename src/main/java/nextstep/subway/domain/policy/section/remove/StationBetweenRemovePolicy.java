...
public class StationBetweenRemovePolicy implements StationRemovePolicy {

    @Override
    public boolean isSatisfied(Sections sections, Station station) {
        return sections.size() > 1
                && sections.contains(station)
                && !sections.equalLastStation(station)
                && !sections.equalFirstStation(station);
    }

    @Override
    public void execute(Sections sections, Station station) {
        sections.removeBetween(station);
