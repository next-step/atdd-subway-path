 ...
 */
@AcceptanceTest
@DisplayName("지하철 노선 구간 비활성화 인수 테스트")
public class SubwaySectionCloseAcceptanceTest extends SubwayLineAcceptanceTest {

    @Autowired private SubwaySectionCloseApiTester closeTester;
     ...
     */
    @Test
    @DisplayName("지하철 노선의 하행 종점역을 비활성하면 노선 상세 조회에서 확인 불가능")
    void addSubwayLineSection() {
        ...
        //when
        closeTester.노선에_구간을_비활성화한다(이호선, 성수역);
     ...
     */
    @Test
    @DisplayName("지하철 노선의 구간이 하나일 때 구간을 비활성화하면 에러 발생")
    void throwWhenExistsDownStation() {
        //given
        SubwayLineResponse 이호선 = 지하철_노선_생성("이호선", "red", 서울대입구역, 강남역, 5).as(SubwayLineResponse.class);

        //when
        ExtractableResponse<Response> response = closeTester.노선에_구간을_비활성화한다(이호선, 강남역);

        //then
        closeTester.구간이_하나뿐인_노선은_비활성화_시도_시에_에러_발생(response);
    ...
    void connectWithoutMiddleWhenCloseAtMiddle() {
        ...
        //when
        closeTester.노선에_구간을_비활성화한다(이호선, 강남역);
