...
public class PathAcceptanceTest extends AcceptanceTest {
    ...
    private void 이호선_삼호선_신분당선_A호선_생성_호출() {
        var 이호선 = LineFixture.generateLineCreateRequest("2호선", "bg-green-600", getStationId("강남역"), getStationId("교대역"), 10L);
        LineSteps.노선_생성_API(이호선);

        var 삼호선 = LineFixture.generateLineCreateRequest("3호선", "bg-amber-600", getStationId("교대역"), getStationId("남부터미널역"), 2L);
        var 삼호선_응답 = LineSteps.노선_생성_API(삼호선);
        ...
        var 신분당선 = LineFixture.generateLineCreateRequest("신분당선", "bg-hotpink-600", getStationId("강남역"), getStationId("양재역"), 10L);
        LineSteps.노선_생성_API(신분당선);

        var A호선 = LineFixture.generateLineCreateRequest("A호선", "bg-black-600", getStationId("건대역"), getStationId("성수역"), 7L);
        var A호선_응답 = LineSteps.노선_생성_API(A호선);
