...
class SectionsTest {
    ...
    void setUp() {
        ...
        구간 = new Sections(List.of(일구간, 이구간));
    }
    ...
    void 성공_구간_등록시_상행역을_기준으로_노선의_가운데에_구간을_등록할_수_있다() {
        ...
        assertThat(일구간.upStation()).isEqualTo(역삼역);
        assertThat(일구간.downStation()).isEqualTo(선릉역);
        assertThat(일구간.distance()).isEqualTo(7L);
    }

    @Test
    void 성공_구간_등록시_하행역을_기준으로_노선의_가운데에_구간을_등록할_수_있다() {
        구간.addSectionInMiddle(역삼역, 양재역, 3L);
        assertThat(이구간.upStation()).isEqualTo(선릉역);
        assertThat(이구간.downStation()).isEqualTo(역삼역);
        assertThat(이구간.distance()).isEqualTo(7L);
    }

    @Test
    void 성공_노선의_구간에서_신규_구간의_상행역이_기존_구간의_상행역과_일치하는_구간을_찾는다() {
        Section matchingSection = 구간.findMatchingSection(강남역);
        assertThat(matchingSection).isEqualTo(일구간);
    }

    @Test
    void 실패_구간_제거시_구간이_한개만_있을경우_구간을_제거할_수_없다() {
        assertThatThrownBy(() -> new Sections(List.of(일구간)).validateDeleteSection(3L))
                .isInstanceOf(ApplicationException.class)
