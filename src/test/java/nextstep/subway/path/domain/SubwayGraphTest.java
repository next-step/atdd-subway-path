...
class SubwayGraphTest {
    ...
    void setUp() {
        ...
        stations = sections.stream()
                .map(it -> it.getStations())
                .flatMap(Collection::stream)
                .distinct()
                .collect(Collectors.toList());

        subwayGraph = new SubwayGraph(sections);
    }
    ...
    void failStationsSame() {
        // when
        assertThatThrownBy(() -> subwayGraph.getShortestPath(강남역, 강남역))
    ...
    void failStationsNotConnected() {
        // when
        assertThatThrownBy(() -> subwayGraph.getShortestPath(강남역, 야탑역))
                .isInstanceOf(RuntimeException.class)
    ...
    void failStationsNotExist() {
        // when
        assertThatThrownBy(() -> subwayGraph.getShortestPath(강남역, 서현역))
                .isInstanceOf(RuntimeException.class)
