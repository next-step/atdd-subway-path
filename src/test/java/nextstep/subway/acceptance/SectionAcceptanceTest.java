...
class SectionAcceptanceTest extends AcceptanceTest {
    ...
    public void setUp() {
        ...
        Map<String, String> lineCreateParams = createLineCreateParams(강남역, 양재역, DEFAULT_DISTANCE);
    ...
    void addLineSectionBetween(){
        ...
        지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(강남역, 논현역, DEFAULT_DISTANCE /2));
        //then
        ExtractableResponse<Response> response = 지하철_노선_조회_요청(신분당선);
        assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value());
    ...
    void addLineSectionBetweenOverDistance(){
        ...
        Long 논현역 = 지하철역_생성_요청("논현역").jsonPath().getLong("id");
        //then
        ExtractableResponse<Response> response = 지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(강남역, 논현역, DEFAULT_DISTANCE *2));
    ...
    void addLineSectionBetweenNotOverlapStation(){
        ...
        //then
        ExtractableResponse<Response> response = 지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(옥수역, 논현역, DEFAULT_DISTANCE));
        assertThat(response.statusCode()).isEqualTo(HttpStatus.BAD_REQUEST.value());
    }
    ...
    void removeLineSection() {
        // given
        Long 정자역 = 지하철역_생성_요청("정자역").jsonPath().getLong("id");
        지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(양재역, 정자역, DEFAULT_DISTANCE /2));
