...
class PathAcceptanceTest extends AcceptanceTest {
    ...
    /**
     * When 출발역과 도착역 사이의 경로를 조회하면
     * Then 두 역 사이의 환승 구간이 있는 최단경로가 반환된다
     */
    @DisplayName("두 역 사이의 최단경로 조회 (환승구간 존재)")
    @Test
    void shortestPathWithTransfer() {
        // when
        var response = 최단경로_조회_요청(교대역, 양재시민의숲역);

        // then
        경로와_거리_검증(response, List.of(교대역, 남부터미널역, 양재역, 양재시민의숲역), 10);
    }
