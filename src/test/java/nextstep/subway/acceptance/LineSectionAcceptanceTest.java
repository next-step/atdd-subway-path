...
class LineSectionAcceptanceTest extends AcceptanceTest {
	...
	void addLineSectionInExistentSection() {
		...
		지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(강남역, 중간역, 5));
	 ...
	 */
	@DisplayName("지하철 노선에 구간을 등록 예외 - 새로운 구간의 하행역은 해당 노선에 등록되어있는 역일 수 없다.")
	@Test
	void addAlreadyExistentStations() {
		//when
		ExtractableResponse<Response> sectionResponse = 지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(양재역, 강남역, 5));
		//then
	...
	/**
	 * when 새로 추가되는 구간의 길이가 기존의 노선보다 길면
	 * Then SECTION_NOT_LONGER_THEN_EXISTING_SECTION 예외발생
	 */
	@DisplayName("지하철 노선에 구간을 등록 예외 - 새로 추가되는 구간은 기존 구간보다 길 수 없다.")
	@Test
	void addLongerThenExistingSection() {
		//when
		Long 중간역 = 지하철역_생성_요청("중간역").jsonPath().getLong("id");
		ExtractableResponse<Response> sectionResponse = 지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(강남역, 중간역, 20));

		//then
		assertThat(sectionResponse.statusCode()).isEqualTo(HttpStatus.BAD_REQUEST.value());
		assertThat(sectionResponse.jsonPath().getList("errorMessages")).contains(SECTION_NOT_LONGER_THEN_EXISTING_SECTION.getMessage());
	}
