...
class LineSectionAcceptanceTest extends AcceptanceTest {
    ...
    private final int LINE_DISTANCE = 10;
    ...
    public void setUp() {
        ...
        Map<String, String> lineCreateParams = createLineCreateParams(강남역, 양재역, LINE_DISTANCE);
    ...
    /**
     * Given 지하철 노선의 기존 구간의 상행역과 같은 지하철 구간을 생성하고
     * When 지하철 노선에 새로운 구간 추가를 요청 하면
     * Then 노선에 새로운 구간이 추가된다
     */
    ...
    /**
     * Given 지하철 노선의 기존 구간의 상행역과 같은 지하철 구간을 추가하고
     * When 지하철 노선에 추가한 구간의 동일한 길이의 새로운 구간 추가를 요청 하면
     * Then 에러가 발생한다
     */
    @DisplayName("지하철 노선에 기존 구간에 같은 길이의 구간을 등록하면 에러가 발생한다")
    @Test
    void addExistLineSectionException() {
        final int HALF_DISTANCE = LINE_DISTANCE / 2;

        // given
        Long 정자역 = 지하철역_생성_요청("정자역").jsonPath().getLong("id");
        지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(강남역, 정자역, HALF_DISTANCE));

        // when
        Long 미금역 = 지하철역_생성_요청("미금역").jsonPath().getLong("id");
        ExtractableResponse<Response> response = 지하철_노선에_지하철_구간_생성_요청(신분당선
                , createSectionCreateParams(강남역, 미금역, HALF_DISTANCE));
    ...
    void addLineSectionIfDistanceGreaterThanOrEqual() {
        ...
        // then
        assertThat(지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(강남역, 고속터미널역, LINE_DISTANCE)).statusCode())
                .isEqualTo(HttpStatus.BAD_REQUEST.value());
        assertThat(지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(강남역, 고속터미널역, LINE_DISTANCE + 1)).statusCode())
                .isEqualTo(HttpStatus.BAD_REQUEST.value());
    }
    ...
    void removeNonexistentLineSection() {
        ...
        // when
        ExtractableResponse<Response> response = 지하철_노선에_지하철_구간_제거_요청(신분당선, 연신내역);
