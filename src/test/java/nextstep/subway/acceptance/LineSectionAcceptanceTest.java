...
class LineSectionAcceptanceTest extends AcceptanceTest {
    ...
    public void setUp() {
        ...
        Map<String, String> lineCreateParams = createLineCreateParams(강남역, 양재역, 4);
    ...
    void addSectionBetweenSections() {
        ...
        ExtractableResponse<Response> response = 지하철_노선_조회_요청(신분당선);
        assertThat(response.statusCode()).isEqualTo(OK.value());
        assertThat(response.jsonPath().getList("stations.id", Long.class)).contains(강남역, 양재역, 판교역, 정자역);
    }
    ...
    void addSectionUpStation() {
        ...
        // when
        ExtractableResponse<Response> response = 지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(논현역, 강남역, 2));
    ...
    /**
     * when 새로운 역을 하행 종점으로 등록하여 구간을 추가할 경우
     * then 새로운 역이 하행 종점역이 되어 노선 조회시 추가한 역을 찾을 수 있다.
     */
     ...
     * then 구간 등록에 실패한다.
     */
    @DisplayName("기존 구간 사이에 구간 등록시, 해당 구간의 크기가 기존 구간 보다 크거나 같다면 등록시 예외가 발생")
    @Test
    void addSectionExceptionTest1() {
        ...
        // when
        ExtractableResponse<Response> response = 지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(판교역, 정자역, 7));
        // then
     ...
     * then 구간 등록에 실패한다.
     */
    @DisplayName("등록하려는 구간의 두 역이 노선에 모두 등록이 되어있다면 등록시 예외가 발생하는 인수테스트")
    @Test
