...
class LineSectionAcceptanceTest extends AcceptanceTest {
     ...
     * Then 노선에 구간이 제거된다
     */
    @DisplayName("지하철 노선에 하행종점역 구간을 제거")
    @Test
    void removeLineSectionAtLast() {
        // given
        Long 정자역 = 지하철역_생성_요청("정자역").jsonPath().getLong("id");
        지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(양재역, 정자역));
     ...
     * Then 노선에 구간이 제거된다
     */
    @DisplayName("지하철 노선에 상행종점역 구간을 제거")
    @Test
    ...
    @Test
    void removeLineSectionAtMiddle() {
        // given
        Long 정자역 = 지하철역_생성_요청("정자역").jsonPath().getLong("id");
        지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(양재역, 정자역));
    ...
    /**
     * Given 지하철 노선에 새로운 구간 추가를 요청 하고
     * When 노선에 등록되어있지 않은 역의 제거를 요청 하면
     * Then 예외가 발생한다.
     */
    @DisplayName("노선에 등록되지 않은 역을 제거")
    @Test
    void removeLineSectionExceptionNonRegister() {
        // given
        Long 정자역 = 지하철역_생성_요청("정자역").jsonPath().getLong("id");
        지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(양재역, 정자역));
