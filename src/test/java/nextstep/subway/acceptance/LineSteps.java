...
public class LineSteps {
	...
	public static Long 지하철_노선_생성_요청(String name, String color, Long upStationId, Long downStationId, int distance) {
		return RestAssured.given().log().all()
			.contentType(MediaType.APPLICATION_JSON_VALUE)
			.body(createSectionCreateParams(name, color, upStationId, downStationId, distance))
			.when()
			.post("/lines")
			.then().log().all().extract()
			.jsonPath().getLong("id");
	...
	private static Map<String, String> createSectionCreateParams(String name, String color, Long upStationId, Long downStationId,
		int distance) {
		Map<String, String> params = new HashMap<>();
		params.put("name", name);
