...
public class LineSteps {
    ...
    public static Long 지하철_노선_생성_요청(String name, String color, Long upStationId, Long downStationId, int distance) {
        ...
        return RestAssured
                ...
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .when().post("/lines")
                .then().extract().as(LineResponse.class).getId();
    }
    ...
    public static ExtractableResponse<Response> 지하철_노선_구간_제거_요청(Long id, Long stationId) {
        return RestAssured
                .given().log().all()
                .when().delete("/lines/{id}/sections?stationId={stationId}", id, stationId)
                .then().log().all().extract();
    }
