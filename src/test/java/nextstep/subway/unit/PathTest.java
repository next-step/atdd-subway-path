...
public class PathTest {
    ...
    void findShortestPath() {
        GraphPath<Station, DefaultWeightedEdge> shortestPath = new PathFinder(sections).getShortestPath(교대역, 양재역);

        int stationCount = shortestPath.getVertexList().size();
        assertThat(stationCount).isEqualTo(3);
        assertThat(shortestPath.getWeight()).isEqualTo(DEFAULT_DISTANCE*(stationCount-1));
    }
    ...
    void findUnconnetedPath() {
        ...
        GraphPath<Station, DefaultWeightedEdge> shortestPath = new PathFinder(sections).getShortestPath(대방역, 양재역);
