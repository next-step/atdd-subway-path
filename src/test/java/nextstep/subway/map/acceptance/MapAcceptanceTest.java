...
public class MapAcceptanceTest extends AcceptanceTest {
    ...
    void loadMap() {
        ...
        // then
        // 지하철 노선도 응답됨
        assertThat(mapResponse.statusCode()).isEqualTo(HttpStatus.OK.value());

        // 지하철 노선도에 노선별 지하철역 순서 정렬됨
        List<LineResponse> lines = mapResponse.as(MapResponse.class).getLines();

        LineResponse line1 = lines.stream().filter(it -> Objects.equals(it.getId(), lineId1)).findFirst().orElseThrow(RuntimeException::new);
        List<Long> lineStationsOfLine1 = line1.getStations().stream().map(it -> it.getStation().getId()).collect(Collectors.toList());
        assertThat(lineStationsOfLine1).containsExactly(stationId1, stationId2, stationId3);

        LineResponse line2 = lines.stream().filter(it -> Objects.equals(it.getId(), lineId2)).findFirst().orElseThrow(RuntimeException::new);
        List<Long> lineStationsOfLine2 = line2.getStations().stream().map(it -> it.getStation().getId()).collect(Collectors.toList());
        assertThat(lineStationsOfLine2).containsExactly(stationId1, stationId4);
    }
