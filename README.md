...
- [x] 시나리오 작성
  - 성공 시나리오
    - Scenario: 경로가 단건일 경우, 조회 성공 
      ...
      Given 해당 지하철 노선은
            상행역 : 종로3가역 / 하행역 : 시청역 / 길이 : 6
            상행역 : 시청역 / 하행역 : 서울역 / 길이 : 10 인 구간들이 존재하는 노선을 생성한다.
      When 종로3가역부터 서울역까지의 경로를 조회하면
      Then 경로에 있는 역 목록은 종로3가역, 시청역, 서울역 순서대로 구성된다.
      Then 구간의 거리는 16이다.
      ````
    - Scenario: 경로가 다건일 경우, 최단 경로 조회 성공
      ````
      Given 상행역 : 종로3가역 / 하행역 : 시청역 / 길이 : 6
            상행역 : 시청역 / 하행역 : 서울역 / 길이 : 10 인 구간들이 존재하는 노선을 생성한다.
      Given 상행역 : 종로3가역 / 하행역 : 종각역 / 길이 : 3
            상행역 : 종각역 / 하행역 : 서울역 / 길이 : 8 인 구간들이 존재하는 노선을 생성한다.
      When 종로3가역부터 서울역까지의 경로를 조회하면
      Then 경로에 있는 역 목록은 종로3가역, 종각역, 서울역 순서대로 구성된다.
      Then 구간의 거리는 11이다.
      ````
    - Scenario: 노선이 다른 경로 조회 성공
      ````
      Given 상행역 : 종로3가역 / 하행역 : 시청역 / 길이 : 6 구간이 존재하는 노선을 생성한다.
      Given 상행역 : 시청역 / 하행역 : 서울역 / 길이 : 10 구간이 존재하는 노선을 생성한다.
      Given 상행역 : 종로3가역 / 하행역 : 종각역 / 길이 : 3
            상행역 : 종각역 / 하행역 : 서울역 / 길이 : 8 인 구간들이 존재하는 노선을 생성한다.
      When 종로3가역부터 서울역까지의 경로를 조회하면
      Then 경로에 있는 역 목록은 종로3가역, 시청역, 서울역 순서대로 구성된다.
  ...
  - 실패 시나리오
    ...
    - Scenario: 출발역이나 도착역이 존재하지 않으면 조회 실패
      ...
      Given 상행역 : 종로3가역 / 하행역 : 시청역 / 길이 : 6
            상행역 : 시청역 / 하행역 : 서울역 / 길이 : 10 인 구간들이 존재하는 노선을 생성한다.
      When 종로5가부터 서울역까지의 경로를 조회하면
      Then "출발역이 존재하지 않습니다."라는 메시지를 반환한다.
      When 종로3가부터 동대문역까지의 경로를 조회하면
