...
## 요구사항 기반 시나리오
- 구간 조회 기능 인수 테스트 
  - 교대역, 강남역과 그 구간(10km)을 포함하는 2호선 을 생성한다.
  - 강남역, 양재역과 그 구간(10km)을 포함하는 신분당선 을 생성한다.
  - 남부터미널역을 생성하고 교대역-남부터미널역 구간(2km)을 포함하는 3호선 을 생성한다.
  - 3호선에 남부터미널역, 양재역 구간(3km)을 추가한다.
  - 교대역에서 양재역까지의 최단 경로 조회 요청한다.
  - 교대역 -> 남부터미널역 -> 양재역 순서와 총 거리 5km 를 조회 응답으로 받는다.
...
- 구간 조회 예외 인수 테스트
  ...
  - 남부터미널역을 생성하고 교대역-남부터미널역 구간(2km)을 포함하는 3호선 을 생성한다.
  - 3호선에 남부터미널역, 양재역 구간(3km)을 추가한다.
  - 존재하지 않는 역의 id 로 구간 조회의 출발점 혹은 도착점으로 최단 경로 조회 요청한다.
  - 예외가 발생한다.

- 구간 조회 연결되지 않은 역 단위 테스트
  - 교대역, 강남역과 그 구간(10km)을 포함하는 2호선 을 생성한다.
  - 강남역, 양재역과 그 구간(10km)을 포함하는 신분당선 을 생성한다.
  - 남부터미널역을 생성하고 교대역-남부터미널역 구간(2km)을 포함하는 3호선 을 생성한다.
  - 3호선에 남부터미널역, 양재역 구간(3km)을 추가한다.
  - 선릉역, 선정릉역과 그 구간(2km)을 포함하는 수인분당선 생성한다.
  - 출발점을 교대역, 도착점을 선릉역으로 최단 경로 조회 요청한다.
  - 교대역과 선릉역은 연결되지 않았으므로 예외가 발생한다.